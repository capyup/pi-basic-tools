import type { DerivedThinkingStep, ThinkingSemanticRole, ThinkingSourceBlock, ThinkingStepsMode, ThinkingSummaryEvent } from "./types.ts";
...
const SINGLE_LINE_STEP_CUE_RE = /^(?:first|next|then|now|after that|finally|lastly)[,:\s]/i;

function isStandaloneActionLine(line: string): boolean {
	const trimmed = line.trim();
	if (!trimmed) return false;
	if (LIST_ITEM_RE.test(trimmed)) return true;
	if (DIRECT_ACTION_START_RE.test(trimmed)) return true;
	if (STANDALONE_LIST_ACTION_RE.test(trimmed)) return true;
	if (SINGLE_LINE_STEP_CUE_RE.test(trimmed)) return true;
	return false;
}
...
export function splitThinkingIntoStepTexts(text: string): string[] {
	...
	for (let index = 0; index < mergedChunks.length; index += 1) {
		...
		// If a chunk has no empty lines but multiple lines, and every line
		// looks like a standalone action/step, split each line into its own step.
